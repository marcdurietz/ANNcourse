...
def viz_rf(weights,it,vis,hid,grid):
    ...
    for x in range(grid[0]):
        for y in range(grid[1]):
            axs[x,y].set_xticks([]);
            axs[x,y].set_yticks([]);
            axs[x, y].text(0.1, 0.9, f"v={vis[x]}\nh={hid[x+y]}", horizontalalignment='left',
     verticalalignment='top', transform=axs[x,y].transAxes, size='small')
            axs[x,y].imshow(weights[:,:,y+grid[1]*x], cmap="RdGy", vmin=-imax, vmax=imax, interpolation=None)
...
def viz_im(weight):
    ...
    plt.xticks([])
    plt.yticks([])
    plt.imshow(weight, cmap='RdGy', vmin=-imax, vmax=imax, interpolation=None)
    plt.savefig("true")
    plt.close('all')
