...
class DeepBeliefNet():    
    ...
    def recognize(self,true_img,true_lbl):
        ...
        pen_lbl_prob = np.hstack((pen, lbl))
        for _ in range(self.n_gibbs_recog):
            top,_ = self.rbm_stack['pen+lbl--top'].get_h_given_v(pen_lbl_prob)
            pen_lbl_prob,pen_lbl = self.rbm_stack['pen+lbl--top'].get_v_given_h(top)
    

        predicted_lbl = pen_lbl[:, -self.n_labels:]
    ...
    def generate(self,true_lbl,name):
        ...
        for _ in range(1):
            init_vis = np.random.binomial(1, 0.5, (n_sample, self.sizes["vis"]))
            _,init_hid = self.rbm_stack['vis--hid'].get_h_given_v_dir(init_vis)
            _,init_pen = self.rbm_stack['hid--pen'].get_h_given_v_dir(init_hid)
            pen_lbl_prob = np.hstack((init_pen,lbl))
            for _ in range(self.n_gibbs_gener):
                top_prob,top = self.rbm_stack['pen+lbl--top'].get_h_given_v(pen_lbl_prob)
                pen_lbl_prob,pen_lbl = self.rbm_stack['pen+lbl--top'].get_v_given_h(top_prob)
                pen_lbl[:,-self.n_labels:] = lbl.copy()
            
            _,hid = self.rbm_stack['hid--pen'].get_v_given_h_dir(pen_lbl[:,:-self.n_labels])
            vis,_ = self.rbm_stack['vis--hid'].get_v_given_h_dir(hid)  
            records.append( [ ax.imshow(vis.reshape(self.image_size), cmap="RdGy", vmin=0, vmax=1, animated=True, interpolation=None) ] )
    ...
    def train_greedylayerwise(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try :
            ...
            """            
            self.rbm_stack["vis--hid"].untwine_weights()
            hid,_ = self.rbm_stack['vis--hid'].get_h_given_v_dir(vis_trainset)
            ...
            """
            self.rbm_stack["hid--pen"].untwine_weights()
            pen,_ = self.rbm_stack['hid--pen'].get_h_given_v_dir(hid)
